...
function sumun_get_attachment_alt( $attachment_ID ) {
...
add_filter( 'render_block', 'sumun_cover_block_alt', 10, 2 );
function sumun_cover_block_alt( $block_content, $block ) {
    ...
    $alt = sumun_get_attachment_alt ( $block['attrs']['id'] );
...
function wpdocs_filter_gallery_img_atts( $atts, $attachment ) {
    if ( !isset($atts['alt']) || !$atts['alt'] ) {
        $atts['alt'] = sumun_get_attachment_alt($attachment->ID);
    }
    if ( !isset($atts['title']) || !$atts['title'] ) {
        $atts['title'] = sumun_get_attachment_alt($attachment->ID);
    }
    return $atts;
}